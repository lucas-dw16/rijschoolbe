                    ...
                    <tbody>
                        @forelse ($voertuigen as $voertuig)
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/60">
                                ...
                                <td class="border-b border-zinc-200 px-4 py-3 text-center dark:border-zinc-800">
                                    <form action="{{ route('instructeurs.voertuigen.destroy', [$instructeur->Id, $voertuig->Id]) }}" method="POST" onsubmit="return confirm('Weet u zeker dat u dit voertuig wilt verwijderen van {{ $instructeur->Naam }}?');">
                                        @csrf
                                        @method('DELETE')
